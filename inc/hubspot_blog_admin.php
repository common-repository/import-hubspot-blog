...
function hbwp_text_field_0_render(  ) { 
	...
	?>
	<input type='text' id="main_api_key" name='hbwp_settings[hbwp_text_field_0]' value='<?php echo $options['hbwp_text_field_0']; ?>'>
	<?php
...
function hbwp_text_field_1_render(  ) { 
	...
	$options = get_option( 'hbwp_settings' );
	?>
	<input type='text' id="total_posts" name='hbwp_settings[hbwp_text_field_1]' value='<?php echo $options['hbwp_text_field_1']; ?>'>
	<?php
		...
		if($array) { 
			...
			if(!post_exists($row['html_title'])){
		     foreach ($row['topic_ids'] as $key) {
		     	...
		     	$topic_name = $topics_array['name']."<br />";
			...
			//print_r($categories_all);
		     $post_arr = array(
		        'post_title' => $row['html_title'], //Title of post
